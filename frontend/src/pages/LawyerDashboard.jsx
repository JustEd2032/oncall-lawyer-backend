  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      ...
      try {
        ...
        // Load lawyer profile
        try {
          const profileRes = await api.get(`/lawyers/${firebaseUser.uid}`);
          if (profileRes?.data) {
        ...
        {activeTab === "profile" && (
          <div className="card fade-up" style={styles.profileCard}>
            ...
            <p style={{ color: "var(--gray-500)", fontSize: "0.875rem", marginBottom: "1.5rem" }}>
              This is what clients see when browsing lawyers.
            </p>
            <div className="form-group">
              <label className="form-label">Specialties</label>
              <input className="form-input" placeholder="Family Law, Immigration, Criminal Law" value={specialties} onChange={e => setSpecialties(e.target.value)} />
              <span style={{ fontSize: "0.75rem", color: "var(--gray-300)" }}>Separate with commas</span>
            </div>
            <div className="form-group">
              <label className="form-label">Hourly Rate (USD)</label>
              <input className="form-input" type="number" placeholder="150" value={hourlyRate} onChange={e => setHourlyRate(e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Bio</label>
              <textarea className="form-input" rows={4} placeholder="Tell clients about your experience..." value={bio} onChange={e => setBio(e.target.value)} style={{ resize: "vertical" }} />
            </div>
            {saveMsg && (
...
const styles = {
  ...
  apptCard: { padding: "1.25rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" },
  apptLeft: { display: "flex", alignItems: "center", gap: "1rem" },
  apptIcon: { width: "40px", height: "40px", borderRadius: "10px", background: "var(--gray-50)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.1rem" },
  ...
  apptRight: { display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" },
  statusSelect: { border: "1.5px solid var(--gray-100)", borderRadius: "var(--radius)", padding: "0.35rem 0.65rem", fontSize: "0.8rem", color: "var(--navy)", background: "var(--white)", cursor: "pointer", fontFamily: "var(--font-body)" },
